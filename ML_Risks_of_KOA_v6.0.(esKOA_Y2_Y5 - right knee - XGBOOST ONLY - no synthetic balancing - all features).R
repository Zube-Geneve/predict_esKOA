#Version history
#
# v1.6 OAI train and int.test & MOST as ext test 
#
# v1.7 NOT using radiographs in the feature list
#
# v1.8 Convert the dataset as wide so the left and right knee information is available in one row per patient. 
#
# v2.0 The TKR info on the other hand will be combined with left and right TKR, as in the paper of Mahmoud et al 2023.
#
# v3.0 We have 45 features selected now.
#
# v3.1 Replicated Mahmoud et al. However, the number of records do not match. need to review the missing values.
#
# v3.2 Addressed the performance tuning.
#
# v3.3 Addressing imbalance, which was not successful using ROSE package.
#
# v3.4 Combined OAI and MOST, then split for training and test.
#
# v3.5 Adding Performance tuning: threshold determining and applying SMOTE Synthetic Minority Oversampling Technique.
#
# v3.6 Applying 8 models (Gradient Boosting Machine (GBM), Random Forest (RF), LASSO, RIDGE, Linear Discriminant Analysis (LDA), 
#       Naive Bayes, and Decision Tree), and applying original data, and 3 different techniques: SMOTE, ADASYN, and DB-SMOTE)
#
# v3.7 Adding Recursive Feature Elimination (RFE)
#
# v3.8 Use only right knee, and use bmi and age as continuous
#
# v3.9 Remove the variables that do not varied. Also remove the arthroscopy as it is collinear with meniscectomy.
#
# v4.0 No longer use factor predictors. Use numeric.
#
# v4.1 Model optimization of training models.
#
# v4.2 Concentrate only in GBS models
#
# v4.3 Trying XGBoost
#
# v4.4 Apply ROC curve and show optimal threshold for each model.
#     Remember, 
#     We created a trained model with hyperparameters tuned for best predictive model. We used trained data for that. 
#     We then applied internal test data to show the model performance. We plotted ROC curve and show the optimal threshold on the plot.
#     We do the same for the external test data. We also used the optimal threshold obtained from internal test data 
#        for the external test data)
#
# v4.5 Direct display outputs to output file 
#
# v4.6 Small changes for the paper to collect the numbers of missing values etc.
#
# v4.7 Try imputing mode/mean value (for categorical/continuous variables) in training dataset.
#     This version also has the Table 1 baseline characteristics for train_OAI.
#
#
# v4.8 Create Table 1 for train, internal test and external test datasets.
#
# v4.9 Time to get the confidence intervals for model performance metrics.
#
# v5.0 Remove all synthetic balancing. Use only imbalanced dataset.
#
# v5.1 Minor changes for outputs
#
# v5.2 Minor changes for importance feature files.Also use AUC from botstrap in the plots.
#
# v6.0 Minor change: Save the models for each year.
#
########## S T A R T   C O D E ##########
...
setwd("~/PhD/42. Project ML prediction of KOA")

# load data


combinedDF <- read.csv('Data/master_knee_hip_wide.dta (after esKOA exclusions).csv', header=T, na.strings=c(""," ","NA"))
...
########### COMMENTING OUT FOR IMPUTATION PURPOSE ###########################
#combinedDF_sub <- na.omit(combinedDF_sub_temp)
...
# Columns to exclude
exclude_cols <- c("bmiv_baseline", "age_baseline", "smoking_packyr_baseline", "womactot_right_baseline", "sf12_mental_baseline", "sf12_physical_baseline",
...
#******* COMMENTING OUT FOR IMPUTATION. THESE BELOW VARIABLES NOT CORRELATED IN THE IMPUTED DATASET************
#cor_value <- r["knee_men_right_baseline", "knee_arth_right_baseline"]
#cor_value

#As knee_men_right_baseline", "knee_arth_right_baseline correlated it makes sense to remove "knee_arth_right_baseline
#because it does not give any value.
# *************   COMMENTING OUT BELOW FOR IMPUTATION AS THEY ARE NOT CORRELATED IN TEH IMPUTED DATASET ****************
#combinedDF_sub$knee_arth_right_baseline <- NULL

#combinedDF_sub_num$knee_arth_right_baseline <- NULL


# miso format

#the below is for factorial
print(str(combinedDF_sub))

print(str(combinedDF_sub_num))
...
paper_path <- "C:/Users/zubey/OneDrive/Documents/PhD/42. Project ML prediction of KOA/Paper/"
...
data_path <- "C:/Users/zubey/OneDrive/Documents/PhD/42. Project ML prediction of KOA/Data/"
tool_path <- "C:/Users/zubey/OneDrive/Documents/PhD/42. Project ML prediction of KOA/Tool/"
...
if (outcomes == "knee_esKOA_new_y2_right") {
  write.csv(importance_df, "feature_importances_xgb_orig_y2.csv", row.names = TRUE)
} else if (outcomes == "knee_esKOA_new_y5_right") {
  write.csv(importance_df, "feature_importances_xgb_orig_y5.csv", row.names = TRUE)
}
...
if (outcomes == "knee_esKOA_new_y2_right") {
  write.csv(importance_df2, "feature_importances_xgb_tuned_y2.csv", row.names = TRUE)
} else if (outcomes == "knee_esKOA_new_y5_right") {
  write.csv(importance_df2, "feature_importances_xgb_tuned_y5.csv", row.names = TRUE)
}
...
if (outcomes == "knee_esKOA_new_y2_right") {
  pdf_file_xgb_orig <- "C:/Users/zubey/OneDrive/Documents/PhD/42. Project ML prediction of KOA/Data/roc_curve_plot_xgb_orig_INT_TEST_OAI_y2.pdf"
} else if (outcomes == "knee_esKOA_new_y5_right") {
  pdf_file_xgb_orig <- "C:/Users/zubey/OneDrive/Documents/PhD/42. Project ML prediction of KOA/Data/roc_curve_plot_xgb_orig_INT_TEST_OAI_y5.pdf"
}
...
# Determine the file path based on the outcome
if (outcomes == "knee_esKOA_new_y2_right") {
  pdf_file_xgb_orig_OAI_opt <- "C:/Users/zubey/OneDrive/Documents/PhD/42. Project ML prediction of KOA/Data/roc_curve_plot_xgb_orig_INT_TEST_OAI_opt_y2.pdf"
} else if (outcomes == "knee_esKOA_new_y5_right") {
  pdf_file_xgb_orig_OAI_opt <- "C:/Users/zubey/OneDrive/Documents/PhD/42. Project ML prediction of KOA/Data/roc_curve_plot_xgb_orig_INT_TEST_OAI_opt_y5.pdf"
}
...
# Determine the file path based on the outcome
if (outcomes == "knee_esKOA_new_y2_right") {
  pdf_file_xgb_orig_MOST <- "C:/Users/zubey/OneDrive/Documents/PhD/42. Project ML prediction of KOA/Data/roc_curve_plot_xgb_orig_MOST_y2.pdf"
} else if (outcomes == "knee_esKOA_new_y5_right") {
  pdf_file_xgb_orig_MOST <- "C:/Users/zubey/OneDrive/Documents/PhD/42. Project ML prediction of KOA/Data/roc_curve_plot_xgb_orig_MOST_y5.pdf"
}
...
# Determine the file path based on the outcome
if (outcomes == "knee_esKOA_new_y2_right") {
  pdf_file_xgb_orig_MOST_opt <- "C:/Users/zubey/OneDrive/Documents/PhD/42. Project ML prediction of KOA/Data/roc_curve_plot_xgb_orig_MOST_opt_y2.pdf"
} else if (outcomes == "knee_esKOA_new_y5_right") {
  pdf_file_xgb_orig_MOST_opt <- "C:/Users/zubey/OneDrive/Documents/PhD/42. Project ML prediction of KOA/Data/roc_curve_plot_xgb_orig_MOST_opt_y5.pdf"
}
